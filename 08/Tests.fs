...
let rec go (instructions: list<string * int>) (machineState:MachineState) (trace:int list) : TerminationCondition =
        ...
        if instructions |> List.tryItem newState.iptr |> Option.isNone then
...
let ``solves 8a example`` () =
    execute Input.inputLines |> should equal ( InfiniteLoop ({ iptr = 4; accum = 5 }, { iptr = 1; accum = 5 }, ("jmp", -3)) )
    
[<Fact>]
let ``solves 8a problem`` () =
    execute Input.input |> should equal (InfiniteLoop ({iptr=344;accum=1137},{iptr=359;accum=1137},("jmp", 15)))
    
[<Fact>]
let ``terminates at the end of a program`` () =
    execute Input.terminatingInput |> should equal (EndOfProgram ({iptr=8;accum=2},{iptr=9;accum=8},("acc",6)))
