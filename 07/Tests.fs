...
let rec go (map: Map<string,string list>) color (seenColors:string list)=
    if seenColors |> Seq.contains color then
        []
    else
        ...
        | None -> [color]
        | Some colors ->
            let results = colors |> Seq.map (fun c -> go map c (color::seenColors))
            let accum = color::(results |> List.concat) |> List.distinct
            accum
...
let ``solves 7a example``() =
    ...
    let result = findOtherColorsContaining map "shiny gold" 
    result |> Seq.length |> should equal 4
    result |> should equal ["bright white"; "light red"; "dark orange"; "muted yellow"]
