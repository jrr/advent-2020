...
type Helpers() =
    ...
    let ``evaluates single addition`` () =
        "5+2" |> evalString evalPart1 |> should equal "7"

    [<Fact>]
    let ``evaluates multiple operations`` () =
        "1+2*3"
        |> evalString evalPart1
        |> should equal "9"
...
//        "1+(2*3)" |> recSolve |> should equal "7"
    ...
    let ``tokenizes simple case`` () =
        "1+2"
        |> tokenize
        |> should equal [ IntLiteral 1L; Plus; IntLiteral 2L ]

    [<Fact>]
    let ``tokenizes larger number`` () =
        "123"
        |> tokenize
        |> should equal [ IntLiteral 123L ]
...
//                 RightParen ]
    ...
    let ``counts parens`` () =
        ...
        "1(2)3"
        |> countParens
        |> should equal (HasParens 1)
        "(1(2)3)"
        |> countParens
        |> should equal (HasParens 2)
